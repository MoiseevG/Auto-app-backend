...
# --- UPDATE PAYMENT STATUS (PUT + PATCH) ---
@record_router.put("/{record_id}/payment-status", response_model=RecordRead)
@record_router.patch("/{record_id}/payment-status", response_model=RecordRead)
def update_payment_status(record_id: int, data: PaymentStatusUpdate, session: Session = Depends(get_session)):
    db_record = session.get(Record, record_id)
    if not db_record:
        raise HTTPException(status_code=404, detail="Record not found")

    # Валидация статуса (защита от мусора и от старого 'Cancelled' с маленькой буквы)
    allowed_statuses = {"PENDING", "PAID", "Cancelled", "CANCELLED"}
    if data.payment_status not in allowed_statuses:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid payment_status. Allowed: {', '.join(allowed_statuses)}"
        )
...
# --- GET PAYMENT STATUS (только чтение статуса) ---
@record_router.get("/{record_id}/payment-status", response_model=RecordRead)
def get_payment_status(record_id: int, session: Session = Depends(get_session)):
    record = session.get(Record, record_id)
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
    return record
